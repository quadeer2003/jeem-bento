-- First delete any existing gmail items
DELETE FROM bento_items WHERE type = 'gmail';

-- Update the constraint on bento_items table to remove 'gmail' as a valid type
ALTER TABLE bento_items 
  DROP CONSTRAINT IF EXISTS bento_items_type_check;

ALTER TABLE bento_items 
  ADD CONSTRAINT bento_items_type_check 
  CHECK (type IN ('photo', 'calendar', 'youtube', 'links', 'screenshots', 'contacts', 'websites'));
